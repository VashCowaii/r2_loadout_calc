const basicsUpdates = {
    expandRowListingInfo(rowsListings,table) {
        ...
        for (let entry of rowsListings) {
          ...
          ? addRow(entry.statName,(entry.statCoverName || addSpaces(entry.statName)),table[entry.statName] + (table[`${entry.statName}CORE`] || 0),!percent || round,(!percent ? "%" : "")) 
          : "";
          updateTooltip(
            entry.statName,
            entry.tooltip,
    ...
    updateMainFromFormulas(returnObject,index) {
        ...
        rowsListings = [//evadeCost,meleeCost
            ...
            {"statName": "flatWPBonus","statCoverName": "Flat Bonus","tooltip":"This is a flat bonus to the base multiplier of your weapon before % bonuses factor in.","relevantTags": ["BaseWeakPointBonus"],"isNotAPercent": true,"roundAnyways": true,"condition": (returnObject.flatWPBonus != 0)},
            ...
            {"statName": "wpHitRate","statCoverName": "WP Hit Rate","tooltip":"This is the rate at which you hit the weak point you specified in the Enemy tab.<br><br>The calculator takes this rate and averages it against your total weak point damage multi that exceeds 1x.<br><br>To change your weak point hit rate, visit the Enemy tab.","relevantTags": [""],"isNotAPercent": false},
            {"statName": "wpAveraged","statCoverName": "Avg WP Multi","tooltip":"This is the average weak point multiplier achieved using the hit rate you specified under settings in the Enemy tab.","relevantTags": [""],"isNotAPercent": true,"roundAnyways": true,"condition": (returnObject.wpAveraged != returnObject.totalWPBonus)},
        ]
        ...
        rowsListings = [//evadeCost,meleeCost
            ...
            {"statName": "firearmColossusATK","statCoverName": "+Colossus ATK","tooltip":"A bonus to your FirearmATK that applies AFTER the conversion base is calculated, but before the physical type multiplier is applied.","relevantTags": ["ColossusATK","ColossusATK%"],"isNotAPercent": true,"roundAnyways": true},
            {"statName": "physicalTypeBonus","statCoverName": "Advantage Type Multi","tooltip":"This is the multiplier gained by using the correct physical type(crush, pierce, burst) against an enemy. If an enemy is a pierce enemy, they are weak to pierce.<br><br><span>The bonus will say it is 10%, but when attacking a colossi, an additional 10% is added.</span><br>You need to adjust the enemy and part selected under the ENEMY tab to get accurate numbers here.","relevantTags": [],"isNotAPercent": false},
        ...
        rowsListings = [
            {"statName": "FireATK","statCoverName": "Base Fire ATK","tooltip":"The amount of flat Fire attribute ATK provided by a weapon substat.","relevantTags": ["FireATK"],"isNotAPercent": true,"condition": (index.FireATK + index.FireATKCORE != 0)},
            {"statName": "FireATK%","statCoverName": "Fire ATK Conversion","tooltip":"The % amount taken from the Conversion Base FirearmATK of your weapon, that is addded to the flat attribute ATK.","relevantTags": ["FireATK%"],"isNotAPercent": false,"condition": (index["FireATK%"] + index["FireATK%CORE"] != 0)},
            {"statName": "FireATK%Bonus","statCoverName": "+Fire ATK%","tooltip":"A % multiplier to the sum total flat Fire attribute ATK of your weapon.","relevantTags": ["FireATK%Bonus"],"isNotAPercent": false,"condition": (index["FireATK%Bonus"] + index["FireATK%BonusCORE"] != 0)},

            {"statName": "ChillATK","statCoverName": "Base Chill ATK","tooltip":"The amount of flat Chill attribute ATK provided by a weapon substat.","relevantTags": ["ChillATK"],"isNotAPercent": true,"condition": (index.ChillATK + index.ChillATKCORE != 0)},
            {"statName": "ChillATK%","statCoverName": "Chill ATK Conversion","tooltip":"The % amount taken from the Conversion Base FirearmATK of your weapon, that is addded to the flat attribute ATK.","relevantTags": ["ChillATK%"],"isNotAPercent": false,"condition": (index["ChillATK%"] + index["ChillATK%CORE"] != 0)},
            {"statName": "ChillATK%Bonus","statCoverName": "+Chill ATK%","tooltip":"A % multiplier to the sum total flat Chill attribute ATK of your weapon.","relevantTags": ["ChillATK%Bonus"],"isNotAPercent": false,"condition": (index["ChillATK%Bonus"] + index["ChillATK%BonusCORE"] != 0)},

            {"statName": "ToxicATK","statCoverName": "Base Toxic ATK","tooltip":"The amount of flat Toxin attribute ATK provided by a weapon substat.","relevantTags": ["ToxicATK"],"isNotAPercent": true,"condition": (index.ToxicATK + index.ToxicATKCORE != 0)},
            {"statName": "ToxicATK%","statCoverName": "Toxic ATK Conversion","tooltip":"The % amount taken from the Conversion Base FirearmATK of your weapon, that is addded to the flat attribute ATK.","relevantTags": ["ToxicATK%"],"isNotAPercent": false,"condition": (index["ToxicATK%"] + index["ToxicATK%CORE"] != 0)},
            {"statName": "ToxicATK%Bonus","statCoverName": "+Toxic ATK%","tooltip":"A % multiplier to the sum total flat Toxin attribute ATK of your weapon.","relevantTags": ["ToxicATK%Bonus"],"isNotAPercent": false,"condition": (index["ToxicATK%Bonus"] + index["ToxicATK%BonusCORE"] != 0)},

            {"statName": "ElectricATK","statCoverName": "Base Electric ATK","tooltip":"The amount of flat Electric attribute ATK provided by a weapon substat.","relevantTags": ["ElectricATK"],"isNotAPercent": true,"condition": (index.ElectricATK + index.ElectricATKCORE != 0)},
            {"statName": "ElectricATK%","statCoverName": "Electric ATK Conversion","tooltip":"The % amount taken from the Conversion Base FirearmATK of your weapon, that is addded to the flat attribute ATK.","relevantTags": ["ElectricATK%"],"isNotAPercent": false,"condition": (index["ElectricATK%"] + index["ElectricATK%CORE"] != 0)},
            {"statName": "ElectricATK%Bonus","statCoverName": "+Electric ATK%","tooltip":"A % multiplier to the sum total flat Electric attribute ATK of your weapon.","relevantTags": ["ElectricATK%Bonus"],"isNotAPercent": false,"condition": (index["ElectricATK%Bonus"] + index["ElectricATK%BonusCORE"] != 0)},
        ]
