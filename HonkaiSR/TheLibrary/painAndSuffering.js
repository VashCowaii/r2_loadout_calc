...
const megaParsingFuckeryPain = {
    ...
    "Update Displayed Energy Bar"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.display,"Display Count")}
    ...
    "Update Hit-Count Energy Bar"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "Left",
            "Past",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Update Hit-Count Energy Bar");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Update Hit-Count Bar(Display)${parseRef.entityClass ? `[${parseRef.entityClass}]` : ""}:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"Target",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.Left,"Left Count")}
            ${getStandardNameDisplay(initialCounter,parseRef.Past,"Past Count")}
            ${getStandardNameDisplay(initialCounter,parseRef.assignState,"State")}
    ...
    "Define Custom String with Value"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "variableName",
            "value",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Define Custom String with Value");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Define Custom String with Value:</div>&nbsp;
            ${parseRef.variableName} = ${parseRef.value?.displayLines ?? parseRef.value}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.scope,"Context")}
    ...
    "Define Variable with Elation Skill Count"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Define Variable with Elation Skill Count");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Define with Elation Skill Count:</div>&nbsp;
            ${parseRef.variableName}
        </div>
        <div class="modifierDetailsBox">
        </div>`;
    },
    "Define Variable with Cumulative Delay"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "variableName",
            // "value",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Define Variable with Cumulative Delay");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Define with Cumulative Delay:</div>&nbsp;
            ${parseRef.variableName}
        </div>
        <div class="modifierDetailsBox">
        </div>`;
    },
    "Define Variable with Battle Goal Param"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "variableName",
            "goalID",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Define Variable with Battle Goal Param");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Define with Battle Goal Parameter:</div>&nbsp;
            ${parseRef.variableName}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.goalID,"Goal ID")}
        </div>`;
    },
    ...
    "Sparxie Test Data"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Sparxie Test Data");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Sparxie Test Data:</div>&nbsp;
    ...
    "Define Custom Variable with HP Loss Est"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "targetDefender",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Define Custom Variable with HP Loss Est");
        // initialCounter++;

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Define with HP Loss Estimate:</div>&nbsp;
            ${parseRef.variableName}
        </div>
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.behaviorTag,"DMG Behavior Tag")}
            
            ${getStandardNameDisplay(initialCounter,parseRef.damagePrecent,"DMG %")}
        </div>
        `;
    },
    ...
    "Define Custom Variable with Array Value"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "index",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Define Custom Variable with Array Value");
        // initialCounter++;

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Define with Array Value:</div>&nbsp;
            ${parseRef.variableName}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"Target",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.index,"Array Index")}
    ...
    "Stage Type"(parseRef,initialCounter) {
        ...
        return `<div class="actionDetailBody2">
            ...
            Type ${parseRef.invertCondition ? "NOT" : ""}=== ${parseRef.stageType ?? "(No type defined)"}
        </div>`;
    },
    ...
    "Set SkillPoint Mechanic"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "valueType",
            "modifierName",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Set SkillPoint Mechanic");
        ...
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Set SkillPoint Mechanic${parseRef.isRemove ? " [REMOVE]" : ""}:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.modifierName,"With Modifier")}
            ${getStandardNameDisplay(initialCounter,parseRef.modifierNameRemoved,"To Remove")}
            ${getStandardNameDisplay(initialCounter,parseRef.displayName,"Display")}
        </div>`;
    },
    "Change Skill Point Max"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.silent,"Silent")}
        </div>`;
    },
    ...
    "Stack Target Application Chance"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            // "resistArray",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Stack Target Application Chance");
        ...
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Stack Target Application Chance:</div>&nbsp;
            on ${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter)}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.statName,"Flag List")}
            ${getStandardNameDisplay(initialCounter,parseRef.value,"Chance")}
        </div>`;
    },
        ...
        if (parseRef.resistArray?.length) {
            for (let resistEntry of parseRef.resistArray) {
                resistString += `<div class="actionDetailBody2Detail">
                    ...
                    ${resistEntry.value?.displayLines ?? resistEntry.value}
                </div>`
            }
        }
    ...
    "Read Variable Value"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.isReset,"Reset[?]")}
        </div>`;
    },
    ...
    "Call Parameter Sequence"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "parameterName",
            // "matchToPathFrom",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Call Parameter Sequence");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Call Parameter Sequence:</div>
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.parameterName,"Parameter Name")}
        </div>`;
    },
    "Check Enemy Team Killed"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "forWaveEnd",
            // "matchToPathFrom",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Check Enemy Team Killed");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Check Enemy Team Killed:</div>
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.forWaveEnd,"For Wave End")}
        </div>`;
    },
    "Trigger: Passive Abilities"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "team",
            // "matchToPathFrom",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Trigger: Passive Abilities");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Trigger: Passive Abilities:</div>
        </div>
        <div class="modifierDetailsBox">
    ...
    "Set Entity Level"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "byStage",
            "target",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Set Entity Level");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Set Entity Level:</div>&nbsp;
            ${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter)}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.byStage,"By Stage Level")}
        </div>`;
    },
    ...
    "Select by Enemy ID"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "idList",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Select by Enemy ID");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Select by Enemy ID:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.idList,"ID List")}
        </div>`;
    },
    "Select by Target Flag"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "flags",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Select by Target Flag");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Select by Target Flag:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.flags,"Flags")}
        </div>`;
    },
    "Select by Modifier Name"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "modifier",
            "target",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Select by Modifier Name");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Select by Modifier Name:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.modifier,"Name")}
            ${getStandardNameDisplay(initialCounter,parseRef.target,"Caster Source",true)}

            ${getStandardNameDisplay(initialCounter,parseRef.max,"Max")}
            ${getStandardNameDisplay(initialCounter,parseRef.tieBreakWithRandom,"Tie-Break: Random")}
            ${getStandardNameDisplay(initialCounter,parseRef.sortOperator,"Sort Operator",true)}
        </div>`;
    },
    "Select by Target Pool"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Select by Target Pool");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Select by Target Pool:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"Target",true)}
    ...
    "Select by Unique Identifier"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "uniqueName",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Select by Unique Identifier");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Select by Unique Identifier:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""} ${parseRef.uniqueName}
        </div>
        <div class="modifierDetailsBox">
        </div>`;
    },
    "Select by Target Stat"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "value",
            "compareType",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Select by Target Stat");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Select by Target Stat:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.value,"Stat")}
            ${getStandardNameDisplay(initialCounter,parseRef.compareType,"Comparison")}
        </div>`;
    },
    "Select by Target Rank"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "compareType",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Select by Target Rank");
        // initialCounter++;
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Select by Target Rank:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.compareType,"Comparison")}
    ...
    "Modify Share Points"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "function",
            "pointsArray",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Modify Share Points");
        // initialCounter++;
        return `<div class="actionDetailBody2Preview"> 
            <div class="rotationConditionOperatorHeaderInline">Modify Share Points:</div>&nbsp;
            ${parseRef.pointsArray ?? ""}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.function,"Function")}
        </div>`;
    },
    "Define Custom Variable with Share Points"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "variableName",
            "sharePointType",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Define Custom Variable with Share Points");
        // initialCounter++;
        return `<div class="actionDetailBody2Preview"> 
            <div class="rotationConditionOperatorHeaderInline">Define with Share Points:</div>&nbsp;
            ${parseRef.variableName}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.sharePointType,"Share Point Type")}
        </div>`;
    },
    ...
    "Define Custom Variable with Modifier Values"(parseRef,initialCounter) {
        ...
        return `<div class="actionDetailBody2Preview"> 
            <div class="rotationConditionOperatorHeaderInline">Define with Modifier Attribute:</div>&nbsp;
            ${parseRef.variableName} = ${parseRef.valueType} * ${parseRef.multiplier?.displayLines ?? parseRef.multiplier} on ${parseRef.modifierName ? `(${parseRef.modifierName})` : ""}${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter) ?? "[Undefined or Context Target]"}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.scope,"Context")}
        </div>`;
    ...
    "Define Custom Property"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.canModifyLimbo,"Can Modify in Limbo")}
        </div>`;
    },
    ...
    "TEST DATA: Assist Skill Energy"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "on",
            "add",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"TEST DATA: Assist Skill Energy");
        // initialCounter++;
        return `<div class="actionDetailBody2Preview"> 
            <div class="rotationConditionOperatorHeaderInline">TEST DATA: Assist Skill Energy:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.on,"Target",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.add,"Value")}
        </div>`;
    },
    "TEST DATA: PARAM STRING PAINTER"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "paramArray",
            // "skillTypesAllow",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"TEST DATA: PARAM STRING PAINTER");
        // initialCounter++;


        let tagStrings = "";
        const paramArray = parseRef.paramArray ?? [];
        for (let string of paramArray) {
            tagStrings += `${getStandardNameDisplay(initialCounter,string,"Tag")}`
        }

        return `<div class="actionDetailBody2Preview"> 
            <div class="rotationConditionOperatorHeaderInline">TEST DATA: PARAM STRING PAINTER:</div>
        </div>
        <div class="modifierDetailsBox">
            ${tagStrings}
        </div>`;
    ...
    "Change Entity Turn-State"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.revertDefault,"Back to Default")}
        </div>`;
    },
    ...
    "Change Battle Arena"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.arenaID,"Arena ID")}
    ...
    "Set IronTomb Progress"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "show",
            "display",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Set IronTomb Progress");
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.warningTheshold,"Warning Theshold")}
    ...
    "Adjust Team Punchline Value"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "value",
            "adjustment",
    ...
    "Sort Sequence"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "stat",
            // "living",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Sort Sequence");
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.TargetList,"Sequence",true)}
    ...
    "Divide HP Bar into Segments"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Divide HP Bar into Segments");
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"Target",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.segCount,"Segment Count")}
    ...
    "Sort by Stat Percentage"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "stat",
            "living",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Sort by Stat Percentage");
    ...
    "Stage ID"(parseRef,initialCounter) {
        ...
        return `<div class="actionDetailBody">
            <div class="rotationConditionOperatorHeaderInline">${parseRef.name}:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""}${parseRef.compareType ?? "[=?]"} ${parseRef.ID}(${parseRef.characterName})
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"Target",true)}
    ...
    "Enemy ID"(parseRef,initialCounter) {
        ...
        return `<div class="actionDetailBody">
            ...
            ${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter)} ${parseRef.invertCondition ? "NOT" : ""}= ${parseRef.ID?.displayLines ?? parseRef.ID}(${parseRef.characterName})
        </div>
        <div class="modifierDetailsBox">
    ...
    "Add Global Weakness" (parseRef,initialCounter){
        const knownKeySet = new Set ([
            "name",
            "type",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Overworld Logic State");
    ...
    "Overworld Logic State" (parseRef,initialCounter){
        const knownKeySet = new Set ([
            "name",
            "type",
            "invertCondition",
    ...
    "Action Advance/Delay"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.isStartingDelay,"Starting Delay")}
    ...
    "Move Input Active (Overworld)"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Move Input Active (Overworld)");
    ...
    "In Motion (Overworld)"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "flag",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"In Motion (Overworld)");
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.flag,"Flag")}
        </div>`;
    },
    "Aha Instant is Paused"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "target",
        ])
    ...
    "Add Target by Summoned Units"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "summonID",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Add Target by Summoned Units");
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.summonID,"Summon ID")}
        </div>`;
    },
    ...
    "Force-Sync AV Link"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "from",
            "to",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Force-Sync AV Link");
        // initialCounter++;

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Force-Sync AV Link:</div>
    ...
    "Create AV Link"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "from",
            "to",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Create AV Link");
        // initialCounter++;

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Create AV Link:</div>
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.linkOrder,"Link Order")}
            ${getStandardNameDisplay(initialCounter,parseRef.isStart,"Start")}

            ${parseRef.linkOwnerModifications != undefined ? `<div class="actionDetailBody2">
                <div class="rotationConditionOperatorHeaderInline">Owner Logic:</div>&nbsp;
                ${parseRef.linkOwnerModifications.displayLines ?? typeof parseRef.linkOwnerModifications === "object" ? megaParsingFuckery.ValuePerStackParsing(parseRef.linkOwnerModifications,initialCounter) : parseRef.linkOwnerModifications}
            </div>` : ""}
            ${parseRef.linkFollowerModifications != undefined ? `<div class="actionDetailBody2">
                <div class="rotationConditionOperatorHeaderInline">Follower Logic:</div>&nbsp;
                ${parseRef.linkFollowerModifications.displayLines ?? typeof parseRef.linkFollowerModifications === "object" ? megaParsingFuckery.ValuePerStackParsing(parseRef.linkFollowerModifications,initialCounter) : parseRef.linkFollowerModifications}
            </div>` : ""}
        </div>`;
    },
    ...
    "Set Warning Countdown"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Set Warning Countdown");
        // initialCounter++;

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Set Warning Countdown:</div>
        </div>
        <div class="modifierDetailsBox">
    ...
    "Lock Battle Actions"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "on",
            "whitelist",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Lock Battle Actions");
        // initialCounter++;

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Lock Battle Actions:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.isLock,"Lock")}
        </div>`;
    ...
    "Team Action"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.activated,"Activated")}
    ...
    "Inject Ability Use"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.isAssistantAbility,"Is Assistant Ability")}
            ${getStandardNameDisplay(initialCounter,parseRef.abilitySource,"Ability Source",true)}
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.silentExecute,"Silent Execution")}
    ...
    "Is Revivable from Limbo"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "invertCondition"
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Is Revivable from Limbo");
    ...
    "HP-Share Group is Dead"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "invertCondition"
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"HP-Share Group is Dead");
    ...
    "Is Character Weakness Activated"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            // "modifier",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Is Character Weakness Activated");
    ...
    "Infinite Enemy Waves Remain"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Infinite Enemy Waves Remain");
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.totalRank,"Total Rank")}
        </div>`;
    },
    "Has Currency Wars Trait"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "trait",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Has Currency Wars Trait");

        // initialCounter++;
        return `<div class="actionDetailBody">
            <div class="rotationConditionOperatorHeaderInline">${parseRef.name}:</div>&nbsp;
            ${parseRef.trait} ${parseRef.invertCondition ? "NOT " : ""}on ${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter)}
        </div>
        <div class="modifierDetailsBox">
        </div>`;
    },
    ...
    "Is Indirect DMG"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "states",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Is Indirect DMG");
    ...
    "In Aha-Instant"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "states",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"In Aha-Instant");
    ...
    "Is Team Leader"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            // "modifier",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Is Team Leader");
    ...
    "Has Modifier (OVERWORLD)"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "buffID",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Has Modifier (OVERWORLD)");
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.buffID,"Buff ID")}
            ${getStandardNameDisplay(initialCounter,parseRef.justAddedOrActive,"Needs Active or Just Added")}
            ${getStandardNameDisplay(initialCounter,parseRef.includePreDeath,"Include Pre-Death")}
            ${getStandardNameDisplay(initialCounter,parseRef.casterFilter,"Caster Filter",true)}
        </div>`;
    ...
    "Check Modifier Death Source"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "source",
            // "target",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Check Modifier Death Source");

        // initialCounter++;
        return `<div class="actionDetailBody">
            <div class="rotationConditionOperatorHeaderInline">Check Modifier Death Source:</div>&nbsp;
            ${parseRef.invertCondition ? "(Inverted) " : ""}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.source,"Source")}
        </div>`;
    },
    "Entity is Flying"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "value",
            // "target",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Entity is Flying");

        // initialCounter++;
        return `<div class="actionDetailBody">
            <div class="rotationConditionOperatorHeaderInline">Entity is Flying:</div>&nbsp;
            ${parseRef.invertCondition ? "(Inverted) " : ""}
        </div>
        <div class="modifierDetailsBox">
        </div>`;
    ...
    "Is First Challenge-Battle"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "target",
            "target",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Is First Challenge-Battle");
    ...
    "Has Enemy On-Wave"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "afterWave",
            "timing",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Has Enemy On-Wave");
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.timing,"Timing")}
            ${getStandardNameDisplay(initialCounter,parseRef.afterWave,"After Wave")}
        </div>`;
    },
    ...
    "Ability Has Tag"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "tag",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Ability Has Tag");

        // initialCounter++;
        return `<div class="actionDetailBody">
            <div class="rotationConditionOperatorHeaderInline">${parseRef.name}:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""} ${parseRef.tag}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"Target",true)}
        </div>`;
    },
    "Is Game Mode"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "target",
            "modes",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Is Game Mode");

        // initialCounter++;
        return `<div class="actionDetailBody">
            <div class="rotationConditionOperatorHeaderInline">${parseRef.name}:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""} ${parseRef.modes}
        </div>`;
    },
    ...
    "Override Enemy Rank"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Override Enemy Rank");
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"Target",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.revert,"Revert")}
        </div>`;
    },
    "Lock Toughness"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "percent",
            // "target",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Lock Toughness");
    ...
    "Deal Toughness DMG"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.bypassBreakMute,"Bypass Break Mute")}
    ...
    "Update Next Wave Enemy"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "context",
            "enemyID",
            "activityGroupScaling",
            "slot",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Update Next Wave Enemy");
        
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Update Next Wave Enemy:</div>
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.context,"Context")}
            ${getStandardNameDisplay(initialCounter,parseRef.enemyID,"Enemy ID")}
            ${getStandardNameDisplay(initialCounter,parseRef.activityGroupScaling,"Activity Group Scaling")}
    ...
    "Override HP Lost"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "setTo",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Override HP Lost");
        
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Override HP Lost:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.setTo,"Set To")}
        </div>
        `;
    },
    ...
    "Array Condition"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "arrayToCheck",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Array Condition");

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Array Condition:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.arrayToCheck,"Checks Array")}
        </div>
        `;
    },
    "Automatically use Ultimate"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.enable,"Enable")}
            ${getStandardNameDisplay(initialCounter,parseRef.binding,"Binding")}
        </div>
        `;
    },
    ...
    "Target Can Take Action"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            // "living",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Target Can Take Action");


        
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Target Can Take Action:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""}${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter)}${parseRef.living ? "(Living)" : ""}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.source,"Source",true)}
    ...
    "TARGET IS: UNKNOWN CHECK 1"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "invertCondition",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"TARGET IS: UNKNOWN CHECK 1");


        
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">TARGET IS: UNKNOWN CHECK 1:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""} ${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter)}${parseRef.living ? "(Living)" : ""}
        </div>
        <div class="modifierDetailsBox">
        </div>
    ...
    "Battle Perform Queued"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "target",
            // "living",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Battle Perform Queued");


        
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Battle Perform Queued:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT " : ""} 
        </div>
        <div class="modifierDetailsBox">
        </div>
    ...
    "Add Target by Enemies in Alert Range"(parseRef,initialCounter) {return megaParsingFuckery["Add Target by Obscure Bullshit"](parseRef,initialCounter)},
    "Add Target by Enemies in Attack Range"(parseRef,initialCounter) {return megaParsingFuckery["Add Target by Obscure Bullshit"](parseRef,initialCounter)},
    "Add Target by Chess Skill Target"(parseRef,initialCounter) {return megaParsingFuckery["Add Target by Obscure Bullshit"](parseRef,initialCounter)},
    "Add Target by Chess Lock Target"(parseRef,initialCounter) {return megaParsingFuckery["Add Target by Obscure Bullshit"](parseRef,initialCounter)},
    "Add Target by Chess Movement Target"(parseRef,initialCounter) {return megaParsingFuckery["Add Target by Obscure Bullshit"](parseRef,initialCounter)},
    "Add Target by Next Ability Caster"(parseRef,initialCounter) {return megaParsingFuckery["Add Target by Obscure Bullshit"](parseRef,initialCounter)},
    ...
    "Toggle Field Targets"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Toggle Field Targets");

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Toggle Field Targets[?]:</div>&nbsp;
            ${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter)}
        </div>
        <div class="modifierDetailsBox">
        </div>
    ...
    "Unbind Unique Name"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "target",
            "uniqueName",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Unbind Unique Name");

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Unbind Unique Name:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.uniqueName,"Unique Identifier")}
    ...
    "Change Enemy Name"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "from",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Change Enemy Name");

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Change Enemy Name:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"To",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.from,"From",true)}
        </div>
        `;
    },
    "Copy Weaknesses"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "from",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Copy Weaknesses");

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Copy Weaknesses:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"To",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.from,"From",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.keepProtected,"Keep Protected Weaknesses")}
        </div>
        `;
    },
    ...
    "Remove Overworld Entity"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "summon",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Remove Overworld Entity");

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Remove Overworld Entity:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.summon,"Unit",true)}
        </div>
        `;
    },
    "Reset Toughness"(parseRef,initialCounter) {
        ...
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Reset Toughness${parseRef.isSet ? " [SET]" : ""}:</div>&nbsp;
            ${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter)}
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.preventWhen0,"Prevent when 0")}
    ...
    "Overworld Attack Instance"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "summonID",
            "onAttack",
        ...
        let refString2 = "";
        let refString3 = "";
        const hasParse = parseRef.onAttack?.length;
        const hasRef = parseRef.onBattle?.length;
        const hasRef2 = parseRef.onHit?.length;
        const hasRef3 = parseRef.onKill?.length;
        if (hasParse) {parseString += megaParsingFuckery.fillEventBodyBox(parseRef.onAttack,initialCounter);}
        if (hasRef) {refString += megaParsingFuckery.fillEventBodyBox(parseRef.onBattle,initialCounter);}
        if (hasRef2) {refString2 += megaParsingFuckery.fillEventBodyBox(parseRef.onHit,initialCounter);}
        if (hasRef3) {refString3 += megaParsingFuckery.fillEventBodyBox(parseRef.onKill,initialCounter);}

        return `
        <details class="rotationsPermaConditionsExpand" open="">
            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                <div class="rotationConditionOperatorHeaderCondition">Overworld Attack Instance:</div>${parseRef.summonID}
            </summary>
            ...
            <div class="rotationConditionOperatorBoxMain">
                ${hasParse ? `<div class="rotationConditionOperatorHeaderConditionTHEN">On Attack</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${parseString}
                </div>` : ""}
                ${hasRef ? `<div class="rotationConditionOperatorHeaderConditionTHEN">On BattleStart</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${refString}
                </div>` : ""}
                ${hasRef2 ? `<div class="rotationConditionOperatorHeaderConditionTHEN">On Hit</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${refString2}
                </div>` : ""}
                ${hasRef3 ? `<div class="rotationConditionOperatorHeaderConditionTHEN">On Kill</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${refString3}
                </div>` : ""}
            </div>
    ...
    "Set LoadState with Animation"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "execute",
            "failed",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Set LoadState with Animation");
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                <div class="rotationConditionOperatorHeaderCondition">Set LoadState with Animation:</div>${parseRef.summonID}
            </summary>
    ...
    "Create Overworld Entity"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "summonID",
            "execute",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Create Overworld Entity");
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                <div class="rotationConditionOperatorHeaderCondition">Create Overworld Entity:</div>${parseRef.summonID}
            </summary>
    ...
    "Overworld Filter Enemies"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "execute",
            // "formula",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Overworld Filter Enemies");
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                <div class="rotationConditionOperatorHeaderCondition">Overworld Filter Enemies:</div>
            </summary>
    ...
    "Overworld Snapshot Event"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "execute",
            "exit",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Overworld Snapshot Event");
        ...
        let refString = "";
        const hasParse = parseRef.execute?.length;
        const hasRef = parseRef.exit?.length;
        if (hasParse) {parseString += megaParsingFuckery.fillEventBodyBox(parseRef.execute,initialCounter);}
        if (hasRef) {refString += megaParsingFuckery.fillEventBodyBox(parseRef.exit,initialCounter);}
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                <div class="rotationConditionOperatorHeaderCondition">Overworld Snapshot Event:</div>
            </summary>
            ...
            <div class="rotationConditionOperatorBoxMain">
                ...
                ${hasRef ? `<div class="rotationConditionOperatorHeaderConditionELSE">EXIT</div>
                    <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                        ${refString}
                    </div>` : ""}
            </div>
    ...
    "Add Battle Event"(parseRef,initialCounter) {
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.canDupe,"Can Duplicate")}
    ...
    "Stack Skill Point Lock-Out"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "amount",
        ])
        ...
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Stack Skill Point Lock-Out:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.amount,"Value")}
        </div>
        `;
    },
    "TEST DATA 1(Binding/skill): ASAT"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "abilityName",
            "auto",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"TEST DATA 1(Binding/skill): ASAT");

        
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">TEST DATA 1(Binding/skill): ASAT:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.abilityName,"Ability")}
        </div>
    ...
    "TEST DATA 2(Binding/skill): ASAT"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "abilityName",
            "auto",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"TEST DATA 2(Binding/skill): ASAT");

        
        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">TEST DATA 2(Binding/skill): ASAT:</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.abilityName,"Ability")}
            ${getStandardNameDisplay(initialCounter,parseRef.auto,"Ability")}
        </div>
        `;
    },
    ...
    "Remove from Team Target Grouping"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.stayInTeam,"Stay in Team Label")}
        </div>
        `;
    },
    ...
    "Can Die Instantly"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "invertCondition",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Can Die Instantly");
    ...
    "Is Entity Special Type"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "target",
            "type",
    ...
    "Detach Parent Mapping Point"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "point",
            "target",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Detach Parent Mapping Point");
    ...
    "Enemy Wave Instance"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "waitForDeath",
            "timing",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Enemy Wave Instance");
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.waitForDeath,"Wait For Death")}
            ${getStandardNameDisplay(initialCounter,parseRef.timing,"Timing")}
            ${getStandardNameDisplay(initialCounter,parseRef.banPassive,"Ban Passive")}
        </div>`;
    },
    "Group has Dying Target"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "entityType",
            "team",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Group has Dying Target");
    ...
    "Compare: Variable"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.from,"From",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.to,"To",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.summoner,"Summoner",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.summonID,"Summon ID")}
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.abilityName,"Ability Name")}
            ${getStandardNameDisplay(initialCounter,parseRef.controlType,"Ability Control Type")}
            ${getStandardNameDisplay(initialCounter,parseRef.modifierName,"Modifier Name")}
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.compareWithMax,"Compare with MAX")}
            ${getStandardNameDisplay(initialCounter,parseRef.sharePointType,"Share Point Type")}
            ${getStandardNameDisplay(initialCounter,parseRef.useCurrentSkill,"Use Current Ability")}
    ...
    "Compare: Ability Value"(parseRef,initialCounter) {
        ...
        return `<div class="actionDetailBody">
            <div class="rotationConditionOperatorHeaderInline">${parseRef.name}:</div>&nbsp;
            ${parseRef.value1.displayLines ?? parseRef.value1} ${parseRef.invertCondition ? "NOT" : ""}${parseRef.compareType} ${parseRef.value2.displayLines ?? parseRef.value2} ${parseRef.target ? `on ${megaParsingFuckery.makeConditionTargetBox(parseRef.target,initialCounter)}` : ""}
        </div>`;
    },
    ...
    "Compare: Ability Use Limit"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "value1",
            "skillName",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Compare: Ability Use Limit");
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.value1,"Delay")}
            ${getStandardNameDisplay(initialCounter,parseRef.skillName,"Ability")}
        </div>`;
    },
    "Compare: Parameter String"(parseRef,initialCounter) {
        ...
        return `<div class="actionDetailBody">
            <div class="rotationConditionOperatorHeaderInline">${parseRef.name}:</div>&nbsp;
            ${parseRef.invertCondition ? "NOT" : ""}${parseRef.compareType} ${parseRef.value2.displayLines ?? parseRef.value2}
        </div>`;
    },
    ...
    "Assign Target Battle-Location"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.target,"Target",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.casterOverride,"Caster Override",true)}
            ${getStandardNameDisplay(initialCounter,parseRef.row,"Row")}
            ${getStandardNameDisplay(initialCounter,parseRef.spot,"Spot")}
    ...
    "Add Events/Bonuses"(parseRef,initialCounter) {
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.ID,"Buff ID")}
                ${getStandardNameDisplay(initialCounter,parseRef.buffLevel,"Buff Level")}
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.asSubModifier,"Add as Sub-Modifier")}
    ...
    "Remove Events/Bonuses"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.removeToBeAdded,"Remove to be Added")}
            ${getStandardNameDisplay(initialCounter,parseRef.overworldID,"Overworld ID")}
        </div>`
    },
    ...
    "AttackScaling"(parseRef,initialCounter) {
        ...
        ${parseRef.DamageElation ? `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Elation DMG:</div>&nbsp;
            ${parseRef.DamageElation?.displayLines ?? parseRef.DamageElation} ${typeof parseRef.DamageType === "object" ? megaParsingFuckery.ValuePerStackParsing(parseRef.DamageType,initialCounter) : `<img src="/HonkaiSR/icon/element/${parseRef.DamageType}.png" class="characterDisplayLogStatIconElement"></img>`}
        </div>` : ""}
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.mortalWound,"Mortal Wound")}
    ...
    "Create Shield"(parseRef,initialCounter) {
        ...
        let parseString = "";
        let refString = "";
        const hasParse = parseRef.onAdd?.length;
        const hasRef = parseRef.onRemove?.length;
        if (hasParse) {parseString += megaParsingFuckery.fillEventBodyBox(parseRef.onAdd,initialCounter);}
        if (hasRef) {refString += megaParsingFuckery.fillEventBodyBox(parseRef.onRemove,initialCounter);}
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.isGlobalShield,"Is Global Shield")}
        </div>
        <div class="rotationConditionOperatorBoxMain">
            ${hasParse ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Added</div>
            <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                ${parseString}
            </div>` : ""}
            ${hasRef ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Removed</div>
            <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                ${refString}
            </div>` : ""}
        </div>
    ...
    "Adjust Shield"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.formula,"Formula")}
            ${getStandardNameDisplay(initialCounter,parseRef.operation,"Operation")}
            ${getStandardNameDisplay(initialCounter,parseRef.value,"Shield Value")}
    ...
    "Set Hit-Class"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "reset",
            "class",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Set Hit-Class");

        return `<div class="actionDetailBody2">
            <div class="rotationConditionOperatorHeaderInline">Set Hit-Class</div>&nbsp;
        </div>
        <div class="modifierDetailsBox">
            ${getStandardNameDisplay(initialCounter,parseRef.reset,"Reset")}
            ${getStandardNameDisplay(initialCounter,parseRef.class,"Class")}
        </div>
        `;
    },
    ...
    "Copy Modifier"(parseRef,initialCounter) {
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.autoRemoveFromSource,"Auto-Remove from Source")}
    ...
    //     `;
    // },
    "Is Weak to Current DMG Hit"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "weakTo",
            "target",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Is Weak to Current DMG Hit");
    ...
    "Target Chess Base"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Target Chess Base");
    ...
    "Inject Extra-Turn"(parseRef,initialCounter) {
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.forcedPunchline,"Forced Punchline Value")}
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.skillTag,"Skill Tag")}
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.sourceOverride,"Source Override",true)}
                ${getStandardNameDisplay(initialCounter,parseRef.target,"Target",true)}
    ...
    "Random Event"(parseRef,initialCounter) {
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.randomTarget,"Random Target",true)}
                ${getStandardNameDisplay(initialCounter,parseRef.count,"Count")}
    ...
    "Target Configuration"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Target Configuration");
    ...
    "Modifier Construction"(parseRef,initialCounter) {
        ...
        let bullshitListerString = "";
        let miscBullshitLister = [
            ["onCreation","On Creation"],
            ["onRemoval","On Removal"],
            ["onStageExit","On Stage Exit"],
            ["onBattleEnd","On Battle End"],
            ["onStageEntry","On Stage Entry"],
            ["onStack","On Stacking"],
            ["onChangeTeamLead","On Team Lead Change"],
            ["onApplication","On Application"],
            ["onBeingAttacked","On Being Attacked"],
            ["onForeGround","On Foreground"],
            ["onBackGround","On Background"],
            ["onTechUse","On Technique Use"],
            ["onBattlePrep","On Battle Prep"],
            ["onTick","On Tick"],
        ...
        for (let bullshit of miscBullshitLister) {
            const key1 = bullshit[0];
            const key2 = bullshit[1];

            knownKeySet.add(key1);

            const checkHas = parseRef[key1];
            if (checkHas?.length) {
                bullshitResult = megaParsingFuckery.fillEventBodyBox(checkHas,initialCounter);

                if (bullshitResult) {
                    bullshitListerString += `<div class="rotationConditionOperatorHeaderConditionTHEN">${key2}</div>
                    <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                        ${bullshitResult}
                    </div>`
                }
        ...
        let functionString2 = "";
        let subModString = "";
        let elationRangeString = "";
        const hasParse = parseRef.execute?.length;
        const hasRef = parseRef.variableValueChange?.length;
        const hasAbilityChange = parseRef.abilityValueChange?.length;
        const hasFunctions = parseRef.modifierFunctions?.length;
        const hasFunctions2 = parseRef.modifierTasks?.length;
        
        const hasSubMods = parseRef.subModList?.length;
        const hasElationRange = parseRef.elationValueChange?.length;
        ...
        if (hasAbilityChange) {abilityString += megaParsingFuckery.fillEventBodyBox(parseRef.abilityValueChange,initialCounter);}
        if (hasFunctions) {functionString += megaParsingFuckery.fillEventBodyBox(parseRef.modifierFunctions,initialCounter);}
        if (hasFunctions2) {functionString2 += megaParsingFuckery.fillEventBodyBox(parseRef.modifierTasks,initialCounter);}
        if (hasSubMods) {subModString += megaParsingFuckery.fillEventBodyBox(parseRef.subModList,initialCounter);}
        if (hasElationRange) {elationRangeString += megaParsingFuckery.fillEventBodyBox(parseRef.elationValueChange,initialCounter);}
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.duration,"Duration")}
                ${getStandardNameDisplay(initialCounter,parseRef.counter,"Counter")}
                ${getStandardNameDisplay(initialCounter,parseRef.tickTime,"Tick Time")}
            ...
            ${hasFunctions2 ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Modifier Tasks</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${functionString2}
                </div>` : ""}
            ...
            ${hasElationRange ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Elation Value Changes</div>
                <div class="rotationsSectionRowHolder${subModString%2 === 0 ? 2 : 1}">
                    ${elationRangeString}
                </div>` : ""}
            ${bullshitListerString}
    ...
    getStringsArrayResult(stringerArray) {
        let returnString = "";
    ...
    "Define Handler Boolean"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "variableName",
            "value",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Define Handler Boolean");
        // stackData
        return `
            <div class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader">
                <div class="rotationConditionOperatorHeaderCondition">${parseRef.name}</div>
            ...
            <div class="modifierDetailsBox">
                ${getStandardNameDisplay(initialCounter,parseRef.variableName,"Name")}
                ${getStandardNameDisplay(initialCounter,parseRef.value,"Value")}  
            </div>
        `;
        // ${parseRef.addedDisplayWeakness} //this one can be a target read as fun as that is
    },
    "Define Handler String"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "variableName",
            "value",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Define Handler String");
        // stackData
        return `
            <div class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader">
                <div class="rotationConditionOperatorHeaderCondition">${parseRef.name}</div>
    ...
    "Check Ability Use Condition"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "conditions",
            "passedValue",
        ...
        return `
            ...
            <div class="modifierDetailsBox">
                ${getStandardNameDisplay(initialCounter,parseRef.passedValue,"Value: Passed")}  
            </div>
        `;
        // ${parseRef.addedDisplayWeakness} //this one can be a target read as fun as that is
    },
    "Action Handling"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "values",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Action Handling");
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                <div class="rotationConditionOperatorHeaderCondition">Action Handling</div>
            </summary>
            ...
            <div class="rotationConditionOperatorBoxMain">
            ${hasParse ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Values</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${parseString}
                </div>` : ""}
    ...
    "HANDLER OPTION"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "option",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"HANDLER OPTION");

        let parseString = "";
        let parseString2 = "";
        // let refString = "";
        const hasParse = parseRef.goal?.length;
        const hasParse2 = parseRef.check?.length;
        // const hasRef = parseRef.whenValueChanges?.length;
        if (hasParse) {parseString += megaParsingFuckery.fillEventBodyBox(parseRef.goal,initialCounter);}
        if (hasParse2) {parseString2 += megaParsingFuckery.fillEventBodyBox(parseRef.check,initialCounter);}
        // if (hasRef) {refString += megaParsingFuckery.fillEventBodyBox(parseRef.whenValueChanges,initialCounter);}

        return `
        <details class="rotationsPermaConditionsExpand" open="">
            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                <div class="rotationConditionOperatorHeaderCondition">HANDLER OPTION</div> ${parseRef.option}
            </summary>

            <div class="modifierDetailsBox">

                ${getStandardNameDisplay(initialCounter,parseRef.weight,"Weight")}
                ${getStandardNameDisplay(initialCounter,parseRef.type,"Type")}

            </div>

            <div class="rotationConditionOperatorBoxMain">
            ${hasParse ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Goal</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${parseString}
                </div>` : ""}
            ${hasParse2 ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Check</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${parseString2}
                </div>` : ""}
            </div>
    ...
    "HANDLER OPTION GROUP"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "option",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"HANDLER OPTION GROUP");
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                <div class="rotationConditionOperatorHeaderCondition">HANDLER OPTION GROUP</div> ${parseRef.option}
            </summary>
            ...
            <div class="rotationConditionOperatorBoxMain">
            ${hasParse ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Option Group List</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${parseString}
                </div>` : ""}
            ${hasParse2 ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Check</div>
    ...
    "Check Ability Use Value"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "skillName",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Check Ability Use Value");
        ...
        <div class="modifierDetailsBox">
            ...
            ${getStandardNameDisplay(initialCounter,parseRef.firstCD,"First Cooldown")}
            ${getStandardNameDisplay(initialCounter,parseRef.regCD,"Cooldown")}
            ${getStandardNameDisplay(initialCounter,parseRef.valueCheck,"Check Value")}
            

        </div>`;
    },
    "Choose Ability Target"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "skillName",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Choose Ability Target");
    ...
    "Sequence Ability Use Value"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "skillName",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Sequence Ability Use Value");
    ...
    "Use Ability Option"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "skillName",
            // "previewValue",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Use Ability Option");
    ...
    "Select Ability Option"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            "fromSource",
            // "previewValue",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Select Ability Option");
        ...
        <div class="modifierDetailsBox">
        ${getStandardNameDisplay(initialCounter,parseRef.fromSource,"From Source")}
        </div>`;
    },
    "Use Sequence Ability Option"(parseRef,initialCounter) {
        const knownKeySet = new Set ([
            "name",
            // "skillName",
            // "previewValue",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Use Sequence Ability Option");
    ...
    "Shot Fired"(parseRef,initialCounter) {
        ...
        let refString = "";
        const hasParse = parseRef.execute?.length;
        const hasRef = parseRef.projectileFinished?.length;
        if (hasParse) {parseString += megaParsingFuckery.fillEventBodyBox(parseRef.execute,initialCounter);}
        if (hasRef) {refString += megaParsingFuckery.fillEventBodyBox(parseRef.projectileFinished,initialCounter);}
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            ${hasRef ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Finished</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${refString}
                </div>` : ""}
            </div>
    ...
    "Mark Entity For Immediate Death"(parseRef,initialCounter) {
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ${getStandardNameDisplay(initialCounter,parseRef.enable,"Enable")}
                ${getStandardNameDisplay(initialCounter,parseRef.dieNow,"Die Now")}
    ...
    "Create Enemy from Custom"(parseRef,initialCounter) {
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.inheritAllProperties,"Inherit All Stats")}
                ${getStandardNameDisplay(initialCounter,parseRef.useType,"Use Type")}
            </div>
        </details>
        `;
        // return `
    ...
    "Combat Buff Selection"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "passed",
        ])
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Combat Buff Selection");
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="rotationConditionOperatorBoxMain">
                ${hasParse ? `<div class="rotationConditionOperatorHeaderConditionTHEN">CHOICES</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${parseString}
                </div>` : ""}
            </div>
    ...
    "BUFF CHOICE"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "lightTarget",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"BUFF CHOICE");
        ...
        // const conditionObject = parseRef.conditions;
        // const conditionName = conditionObject ? conditionObject.name : null;

        // let returnString = "" + (typeof conditionObject === "string" ? `<div class="rotationsConditionsBodyBox">${conditionObject}</div>` : "");
        // const functionExists = megaParsingFuckery[conditionName];
        // if (functionExists) {returnString += `<div class="rotationsConditionsBodyBox">` + functionExists(conditionObject,initialCounter) + `</div>`;}

        // if (conditionObject && !returnString) {
        //     console.log(conditionObject)
        //     throw new Error(`Missing condition display-only definition in IF: ${conditionName}`)
        // }
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ${getStandardNameDisplay(initialCounter,parseRef.lightTarget,"Player Buff Target",true)}
                ${getStandardNameDisplay(initialCounter,parseRef.lightBuff,"Player Buff")}

                ${getStandardNameDisplay(initialCounter,parseRef.darkTarget,"Enemy Buff Target",true)}
                ${getStandardNameDisplay(initialCounter,parseRef.darkBuff,"Enemy Buff")}

                ${getStandardNameDisplay(initialCounter,parseRef.weight,"Auto-Weight")}
            </div>

            <div class="rotationConditionOperatorBoxMain">
                ${hasParse ? `<div class="rotationConditionOperatorHeaderConditionTHEN">When Chosen</div>
                <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                    ${parseString}
                </div>` : ""}
            </div>
    ...
    "Change Character Model"(parseRef,initialCounter) {
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Change Character Model");
    ...
    "Animation Task"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "target",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Animation Task");
    ...
    "Animation Event"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "target",
        ...
        let refString = "";
        const hasParse = parseRef.passed?.length;
        const hasRef = parseRef.moveTasks?.length;
        if (hasParse) {parseString += megaParsingFuckery.fillEventBodyBox(parseRef.passed,initialCounter);}
        if (hasRef) {refString += megaParsingFuckery.fillEventBodyBox(parseRef.moveTasks,initialCounter);}
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="rotationConditionOperatorBoxMain">
                ...
                ${hasRef ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Movement Tasks</div>
                    <div class="rotationsSectionRowHolder${initialCounter%2 === 0 ? 2 : 1}">
                        ${refString}
                    </div>` : ""}
            </div>
    ...
    "Override Priority Tags"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "dynamicStringsArray",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Override Priority Tags");
    ...
    "Sequence Event"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            // "target",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"Sequence Event");
    ...
    "Create Selection List"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "listType",
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ${getStandardNameDisplay(initialCounter,parseRef.listType,"List Type")}
            </div>
    ...
    "FAILURE IF"(parseRef,initialCounter) {
        initialCounter++;
        const knownKeySet = new Set ([
            "name",
            "conditions",
        ...
        megaParsingFuckery.checkKnownKeys(knownKeySet,parseRef,"FAILURE IF");
    ...
    SWITCH(parseRef,initialCounter) {
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            ...
            <div class="modifierDetailsBox">
                ${getStandardNameDisplay(initialCounter,parseRef.switchValue,"Switch Value")}
                ${getStandardNameDisplay(initialCounter,parseRef.compareType,"Compare Cases By")}
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.hasBreak,"Has Break")}
    ...
    "SWITCH CONDITON"(parseRef,initialCounter) {
        ...
        <details class="rotationsPermaConditionsExpand" open="">
            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                <div class="rotationConditionOperatorHeaderCondition">${parseRef.name}</div>
                Case = ${parseRef.caseValueIs?.displayLines ?? parseRef.caseValueIs ?? "[No case, condition based]"}
            </summary>
            ${returnString}
            <div class="rotationConditionOperatorBoxMain">
                ${hasParse ? `<div class="rotationConditionOperatorHeaderConditionTHEN">Execute</div>
    ...
    realTargetData(parseRef,initialCounter) {
        ...
        const mainAbilityString = `
            <details class="rotationsPermaConditionsExpand" open="">
                ...
                ${getStandardNameDisplay(initialCounter,parseRef.allowUnselectable,"Allow Unselectable")}
