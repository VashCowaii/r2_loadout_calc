...
const userTriggers = {
    ...
    updateMainMenuDisplayed(elementID) {
        ...
        if (elementID === 2) {
            readSelection("characterMainCenterDisplayOverview").style.display = "flex";
            readSelection("expandedMiddle").setAttribute("class", "expandedMiddleNoQuarters characterMainCenterDisplayEnemies");
    ...
    updateCharacterUI(statTableRef,currentSlot,silent) {
        ...
        const characterName = enemyData.name;
        const barCount = enemyData.bar;

        // readSelection("characterDisplayPathName").innerHTML = `//<span class="crossThroughTextFaded">${enemyData.image}</span>/${enemyVariantSelected}`;
        let charRef = enemyData;//characters[characterName];
        const charRank = 0;
        ...
        // const newCharRef = charRef.options[charRef.image];
        // console.log(Object.keys(charRef.options).length-1);

        // const optionKeys = Object.keys(charRef.options);
        const newCharRef = charRef.options[enemyVariantSelected];
        // const newCharRef = charRef.options[401501101];
        ...
        for (let traceEntry in newSkillsObject) {
            ...
            // if (currentTraceRefSkill) {
                ...
                for (let innerSkill in currentTraceSlot) {
                    const currentInnerSkill = currentTraceSlot[innerSkill];
                    ...
                    for (let innerSkillVariant in currentInnerSkill) {
                        ...
                        if (currentInnerSkillVariant.extraEffects && Object.keys(currentInnerSkillVariant.extraEffects).length) {
                            ...
                            // let paramString = "";
                            let paramCounter = 0;
                            for (let paramEntry of currentInnerSkillVariant.extraEffects) {
                                const currentEffect = modifiersExtraEffects[paramEntry];
        
        
                                paramString2 += `<div class="rotationsSectionRowHolder3Overview">
                                    <div class="actionDetailBody">
                                        <div class="rotationConditionOperatorHeaderInlineParamsExtraEffectHeader">${currentEffect.name}</div>
                                    </div>
                                    <div class="actionDetailBody">
                                        <div class="actionDetailBody3Description">
                                            ${pagePopulation.cleanDescription(currentEffect.params ?? [],currentEffect.desc ?? "Failed to locate extra effect")}
                                        </div>
                                    </div>
                                </div>`;
                        ...
                        entryString += `
                            <div class="rotationsSectionRowHolder3Overview">
                                <div class="eidolonRowBoxHolderEnemy">

                                    
                                    <div class="eidolonRowIconHolderEnemy">
                                        <img src="/HonkaiSR/${currentInnerSkillVariant.element ? `icon/element/${currentInnerSkillVariant.element}` : "icon/sign/CommonTabIcon"}.png" class="eidolonRowIconEnemy"/>
                                    </div>
                                    ${currentInnerSkillVariant.isAlert ? `<div class="eidolonRowIconHolderEnemy">
                                        <img src="/HonkaiSR/misc/IconBossAlert.png" class="eidolonRowIconEnemyAlert"/>
                                    </div>` : ""}
                                    
                                    <div class="rightDescriptionBoxEidolonsEnemy smallFont">
        ...
        // let levelIndex = 95;
        // let waveSpecificArray = [1,1];
        scalingElite ??= 1;
        const eliteScalars = scaleElite[scalingElite] ?? scaleInfinite[scalingElite];
        // let scaleElite = {
        //     "1": {
        //       "attackScalar": 1,
        //       "defScalar": 1,
        ...
        //       "toughnessScalar": 1
        //     },
        const hardScalars = scaleHard[scalingHard ??= 1][levelIndex ??= 95];
        // "1": {
        //     "attackScalar": 0.64,
        //     "defScalar": 1,
        //     "hpScalar": 0.8,
        ...
        // charRef
        const baseEnemyStats = charRef.stArray;
        const atk = baseEnemyStats[0];
        const def = baseEnemyStats[1];
        const hpb = baseEnemyStats[2];
        const spd = baseEnemyStats[3];
        const spdPl = baseEnemyStats[4] ?? 0;
        const crD = baseEnemyStats[5];
        const efR = baseEnemyStats[6];
        const avD = baseEnemyStats[7];
        const tgh = baseEnemyStats[8];
        const tghPl = baseEnemyStats[9] ?? 0;
        const tghB = baseEnemyStats[10] ?? 1;

        const maskEnemyStats = newCharRef.stArray;
        const atkMASK = maskEnemyStats[0] ?? 1;
        const defMASK = maskEnemyStats[1] ?? 1;
        const hpbMASK = maskEnemyStats[2] ?? 1;
        const spdMASK = maskEnemyStats[3] ?? 1;
        const spdPlMASK = maskEnemyStats[4] ?? 0;
        const tghMASK = maskEnemyStats[5] ?? 1;
        ...
        const finalSPD = +(spd * spdMASK * (eliteScalars.speedScalar ?? 1) * (hardScalars.speedScalar ?? 1) + spdPl + spdPlMASK).toFixed(7);

        const finalCritDMG = +(crD).toFixed(7);

        const finalEffectRES = +(efR + (hardScalars.effectRES ?? 0)).toFixed(7);
        const finalEHR = +(hardScalars.ehr ?? 0).toFixed(7);

        const toughnessBars = tghB;
        const finalToughness = +((tgh * tghMASK * (eliteScalars.toughnessScalar ?? 1) * (hardScalars.toughnessScalar ?? 1) + tghPl + tghPlMASK)/3).toFixed(7);
        ...
        // const finalDEFValue = +(baseEntryData.defBase * eliteScaling.defScalar * hardScaling.defScalar * (eliteData.DefenceModifyRatio?.Value ?? eliteData.DefenceModifyRatio)).toFixed(7);
        // const finalHPValue = +(hpBonusParam * baseEntryData.hpBase * eliteScaling.hpScalar * hardScaling.hpScalar * (eliteData.HPModifyRatio?.Value ?? eliteData.HPModifyRatio)).toFixed(7);
        // const finalSPDValue = +(baseEntryData.speedBase * eliteScaling.speedScalar * hardScaling.speedScalar * (eliteData.SpeedModifyRatio?.Value ?? eliteData.SpeedModifyRatio) + (eliteData.SpeedModifyValue?.Value ?? eliteData.SpeedModifyValue ?? 0)).toFixed(7);
        // const finalToughnessValue = +((baseEntryData.toughnessBase * eliteScaling.toughnessScalar * hardScaling.toughnessScalar * (eliteData.StanceModifyRatio?.Value ?? eliteData.StanceModifyRatio) + (eliteData.StanceModifyValue?.Value ?? eliteData.StanceModifyValue ?? 0))/3).toFixed(7);

        // const finalToughnessBars = baseEntryData.toughnessBars;
        ...
        if (isPhasedHP) {
            ...
            for (let phasedEntry of newCharRef.phhp) {
                console.log(phasedEntry)
                if (!lastKnownValue) {
                    lastKnownValue = phasedEntry;
                    continue;
                }
                else if (phasedEntry != lastKnownValue) {
                    phaseDoesNotMatterHP = false;
                    break;
                }
            }
        }

        const isPhasedToughness = newCharRef.phs?.length;
        let phaseDoesNotMatterToughness = true;
        if (isPhasedToughness) {
            let lastKnownValue = null;
            ...
            for (let phasedEntry of newCharRef.phs) {
                ...
                if (!lastKnownValue) {
                    ...
                    phaseDoesNotMatterToughness = false;
                    break;
                }
            }
        }
        ...
        const isPhasedToughnessBars = newCharRef.phsc?.length;
        let phaseDoesNotMatterToughnessBars = true;
        if (isPhasedToughnessBars) {
            let lastKnownValue = null;

            for (let phasedEntry of newCharRef.phsc) {
                console.log(phasedEntry)
                if (!lastKnownValue) {
                    lastKnownValue = phasedEntry;
                    continue;
                    ...
                    phaseDoesNotMatterToughnessBars = false;
                    break;
                }
            }
        }
        ...
        mainEnemyBoxStringer += ``;

        // <div class="customMenuSearchNote" id="customMenuSearchNote">Opening a Summon's page here, will pass through whatever level/modifiers the Summoner has.</div>
        // mainEnemyBoxStringer += `<div class="enemySummaryDataBoxMain">

        //     <div class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader">
        //             <div class="rotationConditionOperatorHeaderConditionEnemy">SCALARS</div>
        //     </div>
            
        //     <div class="enemyWaveSummaryHolder">
        //         <div class="enemyWaveEnemyDisplayBox clickable">

        //             <img src="/HonkaiSR/enemies/Monster_4012090.png" class="enemyWaveEnemyIcon">

        //             <div class="enemyWaveEnemyDisplayBoxIDLinker clickable" onclick="endgameModeDisplay.linkToEnemy()">
                        
        //                 <span class="enemyLinkerText">Twig of Hundred Eyes</span>
        ...
        // enemyBaseName,enemyMaskName,enemyHardScalar,enemyEliteScalar,enemyWaveScalar
        // enemyData.image}</span>/${enemyVariantSelected
        readSelection("enemyBaseName").innerHTML = enemyData.image;
        readSelection("enemyMaskName").innerHTML = enemyVariantSelected;

        readSelection("enemyHardScalar").innerHTML = scalingHard + ` [LvL ${levelIndex}]`;
        readSelection("enemyEliteScalar").innerHTML = scalingElite;
        ...
        let mainStatString = `<div class="enemyMainBoxOverview">
            <div class="enemyMainStatsBox">
                ...
                <div class="imageRowStatisticBox1 imageRowStatisticBoxEnemyAdjustmentRow">
                    <div class="imageRowStatisticImageBoxEnemyAdjustment"><img src="/HonkaiSR/icon/property/IconMaxHP.png" class="imageRowStatisticImage">
                    <div class="imageRowStatisticNameBox">HP</div>
                    </div>
                    
                    <div class="imageRowStatisticStatBox">${isPhasedHP && !phaseDoesNotMatterHP ? "SEE PHASES" : `${(finalHP ?? 0).toLocaleString()}${barCount > 1 ? ` x${barCount}` : ""}`}</div>
                </div>
                <div class="imageRowStatisticBox2 imageRowStatisticBoxEnemyAdjustmentRow">
                    <div class="imageRowStatisticImageBoxEnemyAdjustment"><img src="/HonkaiSR/icon/property/IconBreakUp.png" class="imageRowStatisticImage">
                    <div class="imageRowStatisticNameBox">Toughness</div>
                    </div>
                    
                    <div class="imageRowStatisticStatBox">${isPhasedToughness && !phaseDoesNotMatterToughness ? "SEE PHASES" : `${(finalToughness ?? 0).toLocaleString()}${toughnessBars > 1 ? ` x${toughnessBars}` : ""}`}</div>
                </div>
                <div class="imageRowStatisticBox1 imageRowStatisticBoxEnemyAdjustmentRow">
                    <div class="imageRowStatisticImageBoxEnemyAdjustment"><img src="/HonkaiSR/icon/property/IconSpeed.png" class="imageRowStatisticImage">
                    <div class="imageRowStatisticNameBox">SPD</div>
                    </div>
                ...
                <div class="imageRowStatisticBox2 imageRowStatisticBoxEnemyAdjustmentRow">
                    ...
                    <div class="imageRowStatisticStatBox">${(finalATK).toLocaleString()}</div>
                </div>
                <div class="imageRowStatisticBox1 imageRowStatisticBoxEnemyAdjustmentRow">
                    <div class="imageRowStatisticImageBoxEnemyAdjustment"><img src="/HonkaiSR/icon/property/IconDefence.png" class="imageRowStatisticImage">
                    <div class="imageRowStatisticNameBox">DEF</div>
                    </div>
                    
                    <div class="imageRowStatisticStatBox">${(finalDEF).toLocaleString()}</div>
                </div>
                <div class="imageRowStatisticBox2 imageRowStatisticBoxEnemyAdjustmentRow">
                    <div class="imageRowStatisticImageBoxEnemyAdjustment"><img src="/HonkaiSR/icon/property/IconStatusResistance.png" class="imageRowStatisticImage">
                    <div class="imageRowStatisticNameBox">Effect RES</div>
                    </div>
                    
                    <div class="imageRowStatisticStatBox">${((finalEffectRES ?? 0) * 100).toLocaleString()}%</div>
                </div>
                <div class="imageRowStatisticBox1 imageRowStatisticBoxEnemyAdjustmentRow">
                    <div class="imageRowStatisticImageBoxEnemyAdjustment"><img src="/HonkaiSR/icon/property/IconStatusProbability.png" class="imageRowStatisticImage">
                    <div class="imageRowStatisticNameBox">Effect Hit Rate</div>
                    </div>
                    
                    <div class="imageRowStatisticStatBox">${((finalEHR ?? 0) * 100).toLocaleString()}%</div>
                </div>
                ${avD != 1 ? `<div class="imageRowStatisticBox2 imageRowStatisticBoxEnemyAdjustmentRow">
                    <div class="imageRowStatisticImageBoxEnemyAdjustment">
                        <div class="imageRowStatisticNameBox">Start AV</div>
                    </div>
                    
                    <div class="imageRowStatisticStatBox">x${(avD ?? 0).toLocaleString()}</div>
                </div>` : ""}
        ...
        mainEnemyBoxStringer += `<div class="enemyMainBoxOverviewColumn">`;

        const summonsChecker = newCharRef.summons;
        // <div class="eidolonRowNameSkill">Summons</div>
        if (summonsChecker?.length) {//rotationsSectionRowHolder1Overview   //enemyConfigSummonRow
            let summonsStringer = `
            <details class="rotationsSectionRowHolder2Overview" open="">

            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                    <div class="rotationConditionOperatorHeaderConditionEnemy">SUMMONS</div>
            </summary>
            <div class="customMenuSearchNote" id="customMenuSearchNote">Opening a Summon's page here, will pass through whatever level/modifiers the Summoner has.</div>
            <div class="enemyWaveSummaryHolder">
                `;
            ...
            for (let summonEntry of summonsChecker) {
                // {
                ...
                summonsStringer += `<a class="enemyWaveEnemyDisplayBox clickable" href="/HonkaiSR/TheLibrary/EnemyConfigs/${summonEntry.base}/?${serialized}" target="_blank">
                    ...
                    <div class="enemyWaveEnemyDisplayBoxIDLinker">
                        
                        <span class="enemyLinkerText">${summonEntry.name}</span>
                        
                        <span>${summonEntry.over}</span>
                    </div>
                </a>`
            }

            summonsStringer += `</div></details>`
            mainEnemyBoxStringer += summonsStringer;
        }
        ...
        // <div class="bigFuckerBoxRow">
        //     <div class="bigFuckerBox">
        //         <div class="imageRowStatisticBox1">
        // <div class="imageRowStatisticImageBox"><img src="/HonkaiSR/icon/property/IconBreakUp.png" class="imageRowStatisticImage"></div>
        // <div class="imageRowStatisticStatBox">T/Phase</div>
        // </div><div class="imageRowStatisticBox3PhaseData">
        //     <span>1:</span>
        //     <div class="imageRowStatisticStatBox">70 x3</div>
        // </div><div class="imageRowStatisticBox3PhaseData">
        //     <span>2:</span>
        //     <div class="imageRowStatisticStatBox">70 x3</div>
        // </div><div class="imageRowStatisticBox3PhaseData">
        //     <span>3:</span>
        //     <div class="imageRowStatisticStatBox">39.97 x9</div>
        // </div>
        //     </div>
        //     <div class="bigFuckerBox">
        //         <div class="imageRowStatisticBox1">
        ...
        // </div><div class="imageRowStatisticBox3PhaseData">
        //     <span>1:</span>
        //     <div class="imageRowStatisticStatBox">10,818,299</div>
        // </div><div class="imageRowStatisticBox3PhaseData">
        //     <span>2:</span>
        //     <div class="imageRowStatisticStatBox">10,818,299</div>
        // </div><div class="imageRowStatisticBox3PhaseData">
        //     <span>3:</span>
        //     <div class="imageRowStatisticStatBox">8,654,639</div>
        // </div>
        //     </div>
        ...
        const hasUsablePhaseData = newEnemyChanger || hasAnyChangedValues
        if (hasUsablePhaseData) {
            // <div class="customMenuSearchNote" id="customMenuSearchNote">Opening a Summon's page here, will pass through whatever level/modifiers the Summoner has.</div>
            let summonsStringer = `
            <details class="rotationsSectionRowHolder1Overview" open="">

            <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                    <div class="rotationConditionOperatorHeaderConditionEnemy">PHASE DATA</div>
            </summary>

            
            <div class="enemyWaveSummaryHolderEnemy">
                `;


            if (hasAnyChangedValues) {

                summonsStringer += `<div class="bigFuckerBoxRow">`
                if ((isPhasedToughness && !phaseDoesNotMatterToughness) || (isPhasedToughnessBars && !phaseDoesNotMatterToughnessBars)) {
                    // toughnessBars
                    summonsStringer += `<div class="bigFuckerBox">
                    <div class="imageRowStatisticBox1">
                        <div class="imageRowStatisticImageBox"><img src="/HonkaiSR/icon/property/IconBreakUp.png" class="imageRowStatisticImagePhased"></div>
                        <div class="imageRowStatisticStatBoxPhased">Toughness</div>
                    </div>`;

                    const adjustToughness = newCharRef.phs;
                    const adjustBars = newCharRef.phsc;

                    console.log(adjustToughness,adjustBars)

                    const loopCount = Math.max((adjustBars?.length ?? 0),(adjustToughness?.length ?? 0));

                    for (let i=0;i<loopCount;i++) {

                        const checkBars = adjustBars?.[i] ?? toughnessBars;
                        const checkToughness = adjustToughness?.[i] ?? 1;

                        const adjustedFinal = +((checkToughness * tgh * tghMASK * (eliteScalars.toughnessScalar ?? 1) * (hardScalars.toughnessScalar ?? 1) + tghPl + tghPlMASK)/3).toFixed(7);

                        summonsStringer += `<div class="imageRowStatisticBox3PhaseDataEnemy">
                            <span>${i+1}:</span>
                            <div class="imageRowStatisticStatBoxEnemyPhased">${(+(adjustedFinal).toFixed(7)).toLocaleString()} x${checkBars}</div>
                        </div>`
                    }
                ...
                // <div class="imageRowStatisticBox3PhaseData">
                //     <span>2:</span>
                //     <div class="imageRowStatisticStatBox">70 x3</div>
                // </div>
                // <div class="imageRowStatisticBox3PhaseData">
                ...
                if ((isPhasedHP && !phaseDoesNotMatterToughness)) {
                    ...
                    summonsStringer += `<div class="bigFuckerBox">
                    <div class="imageRowStatisticBox1">
                        <div class="imageRowStatisticImageBox"><img src="/HonkaiSR/icon/property/IconMaxHP.png" class="imageRowStatisticImagePhased"></div>
                        <div class="imageRowStatisticStatBoxPhased">HP</div>
                    </div>`;

                    const adjustHP = newCharRef.phhp;

                    const loopCount = adjustHP?.length;

                    for (let i=0;i<loopCount;i++) {

                        const checkHP = adjustHP[i] ?? 1;

                        const adjustedFinal = +(checkHP * hpb * hpbMASK * (eliteScalars.hpScalar ?? 1) * (hardScalars.hpScalar ?? 1) * (1 + +(waveSpecificArray?.[1] ?? 0))).toFixed(7);

                        summonsStringer += `<div class="imageRowStatisticBox3PhaseDataEnemy">
                            <span>${i+1}:</span>
                            <div class="imageRowStatisticStatBoxEnemyPhased">${(+(adjustedFinal).toFixed(7)).toLocaleString()}</div>
                        </div>`
                    }
            ...
            if (newEnemyChanger) {
                ...
                for (let z=barCount;z>=1;z--) {
                    const actualPhase = barCount - z + 1;

                    if (!newEnemyChanger[z]) {continue;}
                    const currentChanger = newEnemyChanger[z];



                    summonsStringer += `<details class="rotationsSectionRowHolder2Overview" open="">
                        <summary class="rotationConditionOperatorHeaderAbilityTriggerConditionHeader clickable">
                                <div class="rotationConditionOperatorHeaderConditionEnemy">PHASE ${actualPhase}: Update Enemy</div>
                        </summary>`;


                    let entryString = "";
                    if (newEnemyChanger[z].newName) {
                        // entryString += `<div class="imageRowStatisticBox1">
                        //     <div class="imageRowStatisticStatBox">Name: ${newEnemyChanger[z].newName}</div>
                        // </div>`
                        entryString += `<span class="enemyLinkerText">${newEnemyChanger[z].newName}</span>`
                    }

                    

                    let weaknessIconString = "";
                    for (let weaknessEntry of currentChanger.newWeak) {
                        weaknessIconString += `<img src="/HonkaiSR/icon/element/${weaknessEntry}.png" class="characterDisplayElementWeaknessIcon">`
                    }
                    entryString += weaknessIconString ? `<div class="characterDisplayElementWeaknessIconHolderEnemies">
                        ${weaknessIconString}
                    </div>` : "";


                    let resIconString = "";
                    if (currentChanger.newRes) {
                        resIconString += `<div class="enemyResBoxHolderRow">`
                    }
                    for (let weaknessEntry in currentChanger.res) {
                        resIconString += `<div class="enemyResBoxHolder">
                            <img src="/HonkaiSR/icon/element/${weaknessEntry}.png" class="characterDisplayElementWeaknessIcon">
                            <div>${+(currentChanger.res[weaknessEntry] * 100).toFixed(7)}%</div>
                        </div>`
                    }
                    if (resIconString) {
                        resIconString += `</div>`;
                        entryString += resIconString;
                    }


                    if (currentChanger.newResMod) {
                        let resistanceStringer = "";
                        let resistanceAlternator = 2;
                        for (let resEntry in currentChanger.newResMod) {
                            if (resistanceAlternator === 2) {resistanceAlternator = 0;}
                            resistanceAlternator++;
                            ...
                            const resValue = +(currentChanger.newResMod[resEntry] * 100).toFixed(7)
                            entryString += `<div class="imageRowStatisticBox${resistanceAlternator} imageRowStatisticBoxEnemyAdjustmentRow">
                                <div class="imageRowStatisticImageBoxEnemyAdjustment">
                                <div class="imageRowStatisticNameBox">${resNameConversions[resEntry] ?? resEntry}</div>
                                </div>
                    ...
                    summonsStringer += `<div class="enemyWaveEnemyDisplayBoxWideEnemy">

                        <img src="/HonkaiSR/${enemyImages[enemyData.image]}" class="enemyWaveEnemyIcon">
        ...
        mainEnemyBoxStringer += `</div>` + mainStatString;
        ...
        readSelection("overviewMainBoxHolder").innerHTML = mainEnemyBoxStringer;
        readSelection("overviewMainBoxHolderAbilities").innerHTML = overviewString;
